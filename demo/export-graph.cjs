...
const [side,rows]=sizes.get(n),g=makeMap(side,seed,rows),rotation=process.argv.includes('--rotation'),lines=[`# Fourfold Voronoi map: seed ${seed}, fingerprint ${g.fingerprint}`,rotation?`${g.n}`:`${g.n} ${g.edges.length/2}`];
if(rotation)for(const row of g.rotation)lines.push(`${row.length} ${row.join(' ')}`);
else for(let i=0;i<g.edges.length;i+=2)lines.push(`${g.edges[i]} ${g.edges[i+1]}`);
